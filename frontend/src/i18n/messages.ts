export const messages = {
  ...
  'en-US': {
    ...
    'nav.errorDiagnostics': 'Error diagnostics',
...
const legacyExactEnglishMessages: Record<string, string> = {
  ...
  '追踪上游失败请求的取证记录，定位滥用来源': 'Trace forensic records of failed upstream requests and locate abuse sources',
  '搜索用户 ID...': 'Search user ID...',
  '搜索密钥 ID...': 'Search key ID...',
  '全部类型': 'All types',
  '暂无诊断记录': 'No diagnostic records',
  '筛选时间范围': 'Filter time range',
  '筛选诊断类型': 'Filter diagnostic kind',
  '用户 / 密钥': 'User / key',
  '上游状态': 'Upstream status',
  '信息': 'Message',
  '无信息': 'No message',
  '未知提供商': 'Unknown provider',
  '诊断详情': 'Diagnostic details',
  '请求 ID：': 'Request ID: ',
  '最终状态': 'Final status',
  '格式 / 流式': 'Format / streaming',
  '同步': 'Sync',
  ...
  '尚未生成摘要。点击"生成摘要"调用已配置的 AI 摘要器（未配置时返回 503）。': 'No summary yet. Click "Generate summary" to invoke the configured AI summarizer (returns 503 when it is not configured).',
  '已持久化': 'Persisted',
  '未持久化': 'Not persisted',
  '缓存': 'Cache',
  '诊断标记（error_diagnostic）': 'Diagnostic marker (error_diagnostic)',
  ...
  '请求/响应头': 'Headers',
  '正文': 'Body',
  '该段未捕获任何数据。': 'No data captured for this stage.',
  '空响应': 'Empty response',
  '空响应（200）': 'Empty response (200)',
  '上游 4xx': 'Upstream 4xx',
  '上游 5xx': 'Upstream 5xx',
  ...
  '错误诊断 · AI 摘要': 'Error diagnostics · AI summary',
  '为上游回空 / 4xx / 5xx 诊断事件配置 AI 摘要器，自动总结错误原因': 'Configure the AI summarizer for upstream empty/4xx/5xx diagnostic events to summarize errors automatically',
  'AI 摘要器': 'AI summarizer',
  '配置一个 OpenAI 兼容服务，用于在诊断详情中一键生成错误的自然语言摘要': 'Configure an OpenAI-compatible service to generate natural-language error summaries in diagnostic details',
  '摘要器状态': 'Summarizer status',
  '已配置，可在诊断详情中生成 AI 摘要': 'Configured; AI summaries can be generated in diagnostic details',
  '未配置（保存后生效）': 'Not configured (takes effect after saving)',
  '已启用': 'Enabled',
  '未启用': 'Disabled',
  '超时（秒）': 'Timeout (seconds)',
  '已设置（留空保持不变）': 'Already set (leave empty to keep)',
  'OpenAI 兼容服务的根地址（不含 /chat/completions）': 'Root URL of the OpenAI-compatible service (without /chat/completions)',
  '诊断入口': 'Diagnostics entry',
  '查看上游回空 / 4xx / 5xx 的完整取证记录，支持按会话检索与 AI 摘要': 'View full forensic records of upstream empty/4xx/5xx events, with session search and AI summaries',
  '打开错误诊断': 'Open error diagnostics',
  '加载 AI 摘要器配置失败': 'Failed to load AI summarizer config',
  '保存 AI 摘要器配置失败': 'Failed to save AI summarizer config',
  '请填写 Base URL、模型与 API Key': 'Please fill in Base URL, model and API Key',
  'AI 摘要器配置已保存': 'AI summarizer config saved',
  '当上游对同一会话 / 相同请求持续返回空响应时，本地临时屏蔽该来源，直接回安全审查响应，避免浪费上游配额': 'When the upstream keeps returning empty responses for the same session / identical request, temporarily block the source locally and reply with a safety-review style response to avoid wasting upstream quota',
  '屏蔽策略': 'Shield policy',
  '在统计窗口内达到阈值次回空即触发屏蔽，屏蔽持续设定时长后自动解除': 'Shielding triggers when empty responses reach the threshold within the window, and lifts automatically after the configured duration',
  '启用回空屏蔽': 'Enable empty-response shield',
  '关闭后不记录空响应计数，也不屏蔽任何来源': 'When off, empty responses are not counted and nothing is shielded',
  '触发阈值（次）': 'Trigger threshold (times)',
  '窗口内空响应达到该次数即屏蔽': 'Shield when empty responses reach this count within the window',
  '统计窗口（秒）': 'Counting window (seconds)',
  '计数仅在窗口内有效': 'Counts are only valid within the window',
  '屏蔽时长（秒）': 'Shield duration (seconds)',
  '默认 300 秒（5 分钟）': 'Default 300 seconds (5 minutes)',
  '当前被屏蔽来源': 'Currently shielded sources',
  '屏蔽到期后自动解除；也可手动解除': 'Shields lift automatically on expiry; can also be lifted manually',
  '回空屏蔽未启用': 'Empty-response shield is disabled',
  '当前没有被屏蔽的会话 / 请求': 'No sessions / requests are currently shielded',
  '标识': 'Identifier',
  '剩余时间': 'Remaining time',
  '会话': 'Session',
  ...
  '解除': 'Lift',
  '解除中...': 'Lifting...',
  '加载回空屏蔽配置失败': 'Failed to load shield config',
  '加载屏蔽列表失败': 'Failed to load shielded list',
  '保存回空屏蔽配置失败': 'Failed to save shield config',
  '解除屏蔽失败': 'Failed to lift shield',
  '回空屏蔽配置已保存': 'Shield config saved',
  '已解除屏蔽': 'Shield lifted',
  '即将解除': 'About to lift',
  '成本分层路由': 'Cost-tier routing',
  '按请求上下文长度自动在不同计费模式的上游间切换：小于阈值走一侧、大于阈值走另一侧，利润优先，兼顾会话/缓存粘性。': 'Automatically switch between upstreams of different billing modes by request context length: below threshold one way, above the other; profit first, with session/cache stickiness considered.',
  '启用成本分层': 'Enable cost-tier routing',
  '对该提供商的候选按上下文阈值重排（配合其他提供商的计费模式生效）。': 'Re-rank this provider\'s candidates by the context threshold (works together with other providers\' billing modes).',
  '上下文阈值（tokens）': 'Context threshold (tokens)',
  '阈值以下偏好': 'Below-threshold preference',
  '阈值以上偏好': 'Above-threshold preference',
  '按量计费': 'Per-use billing',
  '按次计费': 'Per-request billing',
  '尊重会话粘性': 'Respect session affinity',
  '在不超出利润让步预算时优先保持同一会话的上游。': 'Prefer keeping the same session\'s upstream while within the profit-sacrifice budget.',
  '尊重缓存粘性': 'Respect cache affinity',
  '在不超出利润让步预算时优先命中缓存亲和的上游。': 'Prefer cache-affine upstreams while within the profit-sacrifice budget.',
  '最大利润让步（USD）': 'Max profit sacrifice (USD)',
  '默认 0（利润绝对优先）': 'Default 0 (profit strictly first)',
  '追踪上游失败请求的取证记录，定位滥用来源；点击行查看完整请求详情': 'Trace forensic records of failed upstream requests to locate abuse sources; click a row for the full request detail',
  '生成/查看 AI 摘要': 'Generate/view AI summary',
  '摘要': 'Summary',
  '重新获取': 'Refetch',
  '强制重新生成': 'Force regenerate',
  '绕过缓存强制重新生成': 'Bypass cache and force regeneration',
  '尚未生成摘要。点击"生成摘要"调用已配置的 AI 摘要器；未配置时请先在 模块管理 → 错误诊断 中完成配置。': 'No summary yet. Click "Generate" to call the configured AI summarizer; if unconfigured, set it up under Module Management → Error Diagnostics first.',
}
