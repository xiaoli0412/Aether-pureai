...
const POST_SNAPSHOT_MIGRATION_VERSIONS: &[i64] = &[
    20260714000000,
    20260714010000,
    20260715000000,
    20260715010000,
...
const POSTGRES_RELAY_PROFIT_REPLAY_DEAD_LETTER_MIGRATION_VERSION: i64 = 20260718020000;
const MYSQL_SQLITE_RELAY_PROFIT_REPLAY_DEAD_LETTER_MIGRATION_VERSION: i64 = 20260718010000;
...
fn relay_profit_replay_dead_letter_migrations_keep_the_same_durable_contract_across_backends() {
    let postgres = POSTGRES_MIGRATOR
        .iter()
        .find(|migration| {
            migration.version == POSTGRES_RELAY_PROFIT_REPLAY_DEAD_LETTER_MIGRATION_VERSION
        })
    ...
    let mysql = super::mysql::MIGRATOR
        .iter()
        .find(|migration| {
            migration.version == MYSQL_SQLITE_RELAY_PROFIT_REPLAY_DEAD_LETTER_MIGRATION_VERSION
        })
    ...
    for (backend, source) in [("postgres", postgres), ("mysql", mysql), ("sqlite", sqlite)] {
        for required_clause in [
            ...
            "DROP INDEX",
            "idx_relay_event_inbox_pending_profit",
            "CREATE INDEX idx_relay_event_inbox_pending_profit",
        ] {
            assert!(
                source.contains(required_clause),
...
#[test]
fn pending_migrations_from_applied_includes_incrementals_after_empty_database_snapshot_stamp() {
    ...
    assert_eq!(
        pending_versions,
        POST_SNAPSHOT_MIGRATION_VERSIONS.to_vec(),
        "fresh snapshot-stamped databases should apply incrementals after the snapshot cutoff"
    );
}
...
async fn prepare_database_for_startup_bootstraps_clean_database() {
    ...
    assert_eq!(
        pending
            ...
            .collect::<Vec<_>>(),
        POST_SNAPSHOT_MIGRATION_VERSIONS.to_vec(),
        "fresh snapshots should report migrations introduced after the snapshot cutoff"
    );
...
async fn prepare_database_for_startup_bootstraps_when_only_unrelated_public_tables_exist() {
    ...
    assert_eq!(
        pending
            ...
            .collect::<Vec<_>>(),
        POST_SNAPSHOT_MIGRATION_VERSIONS.to_vec(),
        "unrelated public tables should still bootstrap the snapshot and expose pending incrementals"
    );
...
async fn sqlite_relay_profit_replay_migration_backfills_existing_inbox_rows() {
    ...
    sqlx::raw_sql(include_str!(
        "../../../../adapters/sqlite/migrations/20260714000000_add_relay_engine_tables.sql"
    ))
    .execute(&pool)
    .await
    ...
    sqlx::raw_sql(include_str!(
        "../../../../adapters/sqlite/migrations/20260717000000_add_relay_event_inbox_profit_replay_status.sql"
    ))
    .execute(&pool)
    .await
    ...
    sqlx::raw_sql(include_str!(
        "../../../../adapters/sqlite/migrations/20260718010000_add_relay_event_inbox_profit_replay_dead_letter.sql"
    ))
    .execute(&pool)
    .await
